...
__stdcall void callbackPatientChange(void *ptr) {
  auto &handler = *reinterpret_cast<Service *>(ptr);
  handler.onPatientsChanged();
}
__stdcall void callbackError(void *ptr, const char *msg) {
  auto &handler = *reinterpret_cast<Service *>(ptr);
  handler.onError(msg);
}
__stdcall void callbackStateChange(void *ptr, int state, double until) {
  auto &handler = *reinterpret_cast<Service *>(ptr);
